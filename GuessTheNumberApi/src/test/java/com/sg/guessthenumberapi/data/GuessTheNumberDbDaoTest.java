...
public class GuessTheNumberDbDaoTest {
    ...
    public void setUp() {
        ...
        final String deleteRoundsTable = "drop table if exists rounds;";
        final String deleteGamesTable = "drop table if exists games;";
        final String createGamesTable = "create table games(gameId int primary key auto_increment, isGameOver boolean not null default 0, targetNum int not null);";
        ...
        jdbc.execute(deleteRoundsTable);
        jdbc.execute(deleteGamesTable);
        jdbc.execute(createGamesTable);
        jdbc.execute(createRoundsTable);
        jdbc.update("INSERT INTO games(gameId, isGameOver, targetNum) "
                + "VALUES(?,?,?)", "1", true, "1234");
        jdbc.update("INSERT INTO games(gameId, isGameOver, targetNum) "
                + "VALUES(?,?,?)", "2", false, "5678");
        jdbc.update("INSERT INTO rounds(roundId, roundNumber, guessNumber, gameId, exactCorrect, partialCorrect, timeStamp) "
